...
class ContestService:
    ...
    def get_leaderboard_data(self):
        ...
        for p in participants:
            ...
            if c.start_time:
                # If finished, use diff. If active, use current duration? 
                # Usually leaderboard shows time taken for finished people.
                if c.status == 'COMPLETED' and c.end_time:
                     diff = c.end_time - c.start_time
                     minutes = int(diff.total_seconds() / 60)
                     entry['time_taken'] = f"{minutes} min"
                elif c.status == 'ACTIVE':
                     diff = datetime.now() - c.start_time
                     minutes = int(diff.total_seconds() / 60)
                     entry['time_taken'] = f"{minutes} min (Running)"
